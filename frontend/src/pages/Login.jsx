...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || t('login.gagal'))
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: '480px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '48px' }}>
          <div style={{
            width: '100px', height: '100px', background: 'var(--orange)',
            border: '6px solid var(--black)', boxShadow: '8px 8px 0 var(--black)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            margin: '0 auto 24px',
          }}>
        ...
        <div className="card-pixel" style={{ overflow: 'hidden' }}>
          <div style={{ padding: '20px 28px', background: 'var(--black)', borderBottom: '4px solid var(--black)' }}>
            <span className="font-title" style={{ fontSize: '20px', color: 'white', letterSpacing: '0.1em' }}>{t('login.title').toUpperCase()}</span>
          </div>
          <form onSubmit={handleSubmit} style={{ padding: '36px', display: 'flex', flexDirection: 'column', gap: '28px' }}>
            {error && (
              <div style={{ padding: '16px 20px', background: '#fdf2f2', border: '4px solid #c0392b', fontSize: '13px', color: '#c0392b', fontFamily: 'Dogica', boxShadow: '4px 4px 0 #c0392b', lineHeight: '1.6' }}>
                ⚠ {error}
              </div>
            )}
            <div>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                placeholder="KAMU@EMAIL.COM" required style={inputStyle} />
            </div>
            <div>
            ...
            <button type="submit" disabled={loading} className="btn-pixel" style={{ width: '100%', justifyContent: 'center', marginTop: '12px' }}>
              <Zap size={18} strokeWidth={3} /> {loading ? (lang === 'id' ? 'MEMUAT...' : 'LOADING...') : t('login.submit').toUpperCase()}
            </button>
          </form>
        </div>
        ...
        <p className="font-pixel" style={{ textAlign: 'center', marginTop: '28px', fontSize: '13px', color: 'var(--black)', letterSpacing: '0.05em' }}>
          ...
          <Link to="/register" style={{ color: 'var(--orange)', textDecoration: 'none', fontWeight: 'bold' }}>{t('register.title').toUpperCase()}</Link>
        </p>
      </div>
    </div>
  )
